...
def create_updated_at_trigger_helper() -> None:
    op.execute(
        """
...
def timestamps_helper() -> Tuple[sa.Column, sa.Column]:
    return (
        sa.Column(
            "created_at",
...
def create_users_table_helper() -> None:
    op.create_table(
        "users",
...
def create_followers_to_followings_table_helper() -> None:
    op.create_table(
...
def create_articles_table_helper() -> None:
    op.create_table(
        "articles",
        sa.Column("id", sa.Integer, primary_key=True),
...
def create_tags_table_helper() -> None:
    op.create_table("tags", sa.Column("tag", sa.Text, primary_key=True))


def create_articles_to_tags_table_helper() -> None:
    op.create_table(
        "articles_to_tags",
        sa.Column(
            "article_id",
...
def create_favorites_table_helper() -> None:
    op.create_table(
        "favorites",
        sa.Column(
            "user_id",
...
def create_commentaries_table_helper() -> None:
    op.create_table(
        "commentaries",
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("body", sa.Text, nullable=False),
...
def add_tags_helper(tags, article_id):
...
def create_new_user_helper(username, email, password, admin=False, image="") -> None:
...
def create_new_article_helper(slug, title, description, body, author_id, tags=[]) -> None:
    connection = op.get_bind()
    res = connection.execute(
        f"""
    ...
    for row in res:
        if tags:
            add_tags_helper(tags, row['id'])


def create_new_comment_helper(body, author_id, article_id) -> None:
    op.execute(
        f"""       
...
def upgrade_helper() -> None:
    ...
    create_new_article_helper(slug="Dev_updates_1", title="Dev updates #1",
                       ...
                       author_id="2",
                       tags = ["dev", "updates"])
    create_new_article_helper(slug="Dev_updates_2", title="Dev updates #2",
                       ...
                       author_id="2",
                       tags=["dev", "updates"])
    create_new_user_helper(username="Hodor", email="holdthedoor@checkmarx.com", password="NoSecIssues", image="https://pyxis.nymag.com/v1/imgs/9bc/c6e/b9ba697b64de36b21e4d2dfb1755b20bbb-23-got-ep-5-002.rsquare.w700.jpg")
    create_new_article_helper(slug="Dev_updates_3", title="Dev updates #3",
    ...
    create_new_article_helper(slug="Dev_updates_4", title="Dev updates #4",
                       ...
                       author_id="3",
                       tags=["security", "dev", "updates"])
    create_new_article_helper(slug="I am Pikachu!", title="I am Pikachu!", description="I am the only Pikachu here, you cant have it!",
    ...
    create_new_user_helper(username="Blastoise", email="Blastoise@checkmarx.com", password="powerfulwater", image="https://www.serebii.net/dungeonrescueteamdx/pokemon/009.png")
    create_new_user_helper(username="Dragonite", email="Dragonite@checkmarx.com", password="firebomb", image="https://static.wikia.nocookie.net/pkmnshuffle/images/a/a6/Dragonite.png/revision/latest?cb=20170407191605")
    create_new_user_helper(username="Gengar", email="Gengar@checkmarx.com", password="ghostly", image="https://i.pinimg.com/736x/54/2c/7f/542c7f7e89f0deb1186bbf9242ebc3ae.jpg")
    create_new_article_helper(slug="Gotta Catch ’Em All!", title="Gotta Catch ’Em All!",
